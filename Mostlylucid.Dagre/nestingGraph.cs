...
public class NestingGraph
{
    ...
    public static void Run(DagreGraph g)
    {
        ...
        foreach (var child in g.Children()) Dfs(g, root, nodeSep, weight, height, depths, child);
    ...
    private static NodeLabel generateEmptyWidHei()
    {
    ...
    public static void Dfs(DagreGraph g, string root, int nodeSep, int weight, int height,
        Dictionary<string, int> depths, string v)
    {
    ...
    public static Dictionary<string, int> TreeDepths(DagreGraph g)
    {
        ...
        foreach (var v in g.Children()) dfs(v, 1);
        return depths;
    }
}