...
public class Order
{
    ...
    public static void _order(DagreGraph g, Action<float> progress = null)
    {
        ...
        // Build layer graphs for each rank.
    ...
    public static void SweepLayerGraphs(DagreGraph[] layerGraphs, bool biasRight)
    {
        ...
        foreach (var lg in layerGraphs)
        {
            ...
            var sorted = SortSubGraph.SortSubraph(lg, root, cg, biasRight);
            var vs = sorted.Vs;
            var length = vs.Count;
    ...
    public static string[][] InitOrder(DagreGraph g)
    {
        ...
        if (hasRank)
        {
            ...
            for (var i = 0; i < maxRank + 1; i++) layers.Add(new List<string>());
    ...
    public static DagreGraph BuildLayerGraph(DagreGraph g, int rank, string relationship)
    {
        ...
        foreach (var v in g.Nodes())
        {
            ...
            rtag = node.RankTag;
            if ((rtag.Rank != null && rtag.Rank == rank) || (rtag.MinRank != null && rtag.MaxRank != null &&
                                                             rtag.MinRank <= rank && rank <= rtag.MaxRank))
            {
                graph.SetNode(v);
                ...
                if (edgeVals != null)
                    ...
                    {
                        var u = e.v == v ? e.w : e.v;
                        var existingEdge = graph.EdgeRaw(u, v);
    ...
    {
        var cc = 0;
        for (var i = 1; i < layering.Count; ++i) cc += TwoLayerCrossCount(g, layering[i - 1], layering[i]);
        return cc;
    }
    ...
    public static int TwoLayerCrossCount(DagreGraph g, string[] northLayer, string[] southLayer)
    {
        ...
        for (var idx = 0; idx < southLayer.Length; idx++) southPos[southLayer[idx]] = idx;
        ...
        foreach (var entry in southEntries)
        {
            ...
            while (index > 0)
            {
                if ((index & 1) != 0) weightSum += tree[index + 1];
                index = (index - 1) >> 1;
                tree[index] += w;
            }
