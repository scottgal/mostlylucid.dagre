...
public static class ParentDummyChains
{
    ...
    private static PathResult findPath(DagreGraph g, Dictionary<string, PostorderEntry> postorderNums, string v,
        string w)
    {
        ...
        parent = w;
        while ((parent = g.Parent(parent)) != lca) wPath.Add(parent);
        wPath.Reverse();
    ...
    private static Dictionary<string, PostorderEntry> postorder(DagreGraph g)
    {
        ...
        {
            var low = lim;
            foreach (var item in g.Children(v)) dfs(item);
