...
public class ResolveConflicts
{
    ...
    public static OrderEntry[] ResolveConflictsMethod(OrderEntry[] entities, DagreGraph cg)
    {
        ...
        foreach (var entry2 in mappedEntries.Values)
            if (entry2.Indegree == 0)
                sourceSet.Add(entry2);
        ...
        while (sourceSet.Count != 0)
        {
            ...
            foreach (var wEntry in entry.OutEntries)
            {
                ...
                if (--wEntry.Indegree == 0) sourceSet.Add(wEntry);
            }
        }
