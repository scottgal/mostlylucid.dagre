...
public class SortSubGraph
{
    ...
    public static void MergeBarycenters(OrderEntry target, OrderEntry other)
    {
        ...
        {
            target.Barycenter =
                (target.Barycenter.Value * target.Weight.Value + other.Barycenter.Value * other.Weight.Value) /
    ...
    public static OrderEntry SortSubraph(DagreGraph g, string v, DagreGraph cg, bool biasRight)
    {
        ...
        foreach (var entry in barycenters)
            if (g.HasChildren(entry.V))
            {
                ...
                if (subgraphResult.Barycenter.HasValue) MergeBarycenters(entry, subgraphResult);
            }

        var entries = ResolveConflicts.ResolveConflictsMethod(barycenters, cg);
        // expand subgraphs
        ...
        if (bl != null)
        {
            ...
            if (g.PredecessorCount(bl) != 0)
            {
                ...
                result.Barycenter = (result.Barycenter.Value * result.Weight.Value + blPred.Order + brPred.Order) /
